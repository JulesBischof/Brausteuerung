...
const recipe_db = {
    ...
    connect: function() {
        this.db  = new sqlite3.Database("./database/recipes.db", sqlite3.OPEN_READWRITE, (err) => {
            if(err) {
    ...
    updateOrInsertRows: async function (tableName, selectedMasterdata, newData, primaryKey = 'id', selectionkey = 'recipe_master_id', fetchmethod = 'all') {
        ...
        try {
            ...
            for (let i = 0; i < numRowsToUpdate; i++) {
                ...
                const values = [...columns.filter(column => column !== primaryKey).map(column => updatedData[column]), currentRow[primaryKey]];
            ...
            for (let i = numRowsToUpdate; i < numRowsToUpdate + numRowsToCreate; i++) {
            ...
            for (let i = numRowsToUpdate + numRowsToCreate; i < numRowsToUpdate + numRowsToCreate + numRowsToDelete; i++) {
