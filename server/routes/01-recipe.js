...
router
  ...
  .delete(async (req, res) => {
    try {
      ...
      await recipe_db.deleteMasterRows('recipe_hops', 'recipe_master_id', selected_id);
      await recipe_db.deleteMasterRows('recipe_rests', 'recipe_master_id', selected_id);
      await recipe_db.deleteMasterRows('recipe_grist', 'recipe_master_id', selected_id);
  ...
  .get(async (req, res) => {
    ...
    try {
      ...
      res_array[0] = await recipe_db.sqlQuery(`SELECT * 
                                               FROM recipe_master 
                                               WHERE id = ?` ,
                                               selected_id, 
                                               'get'
      ...
      res_array[1] = await recipe_db.sqlQuery(`SELECT * 
                                              FROM recipe_rests 
      ...
      res_array[2] = await recipe_db.sqlQuery(`SELECT * 
                                              FROM recipe_hops 
                                              WHERE recipe_master_id = ?` ,
                                              selected_id, 
                                              'all'
      ...
      res_array[3] = await recipe_db.sqlQuery(`SELECT * 
                                              FROM recipe_grist 
                                              WHERE recipe_master_id = ?` ,
                                              selected_id, 
                                              'all'
  ...
  .put(async (req, res) => {
    ...
    try {
      ...
      console.log("_____________________________________________recieved data START: ");
      console.log("_____________________________________________SQLite method: UPDATE");
      console.log("masterdata: ", masterdata);
      console.log("restdata: ", restdata);
      console.log("boildata: ", boildata);
      console.log("gristdata: ", gristdata);
      console.log("_____________________________________________recieved data END")

      await recipe_db.updateOrInsertRows('recipe_master', selectedMasterdata, [masterdata], 'id', 'id', 'all'); //function needs an array to work propery ;) [masterdata] 
      await recipe_db.updateOrInsertRows('recipe_rests', selectedMasterdata, restdata);
      await recipe_db.updateOrInsertRows('recipe_hops', selectedMasterdata, boildata);
      await recipe_db.updateOrInsertRows('recipe_grist', selectedMasterdata, gristdata);
...
router
    ...
    .post(async (req, res) => {
        try {
            ...
            console.log("_____________________________________________recieved data START: ");
            console.log("_____________________________________________SQLite method: INSERT");
            console.log("masterdata: ", masterdata);
            console.log("restdata: ", restdata);
            console.log("boildata: ", boildata);
            console.log("gristdata: ", gristdata);
            ...
            const SQL4id_res_Array = await recipe_db.sqlQuery('SELECT MAX(id) AS id FROM recipe_master', [], 'all');
            const newMasterId = SQL4id_res_Array[0].id; //first entry contains id
            console.log("New Master_ID:", newMasterId);
            console.log("____________________________________________START insert data into database")
            ...
            for (let index = 0; index < restdata.length; index++) {
                ...
                const values = [newMasterId, restNumber, restrow.name, restrow.temperature, restrow.duration];
            ...
            for (let index = 0; index < boildata.length; index++) {
                ...
                const values = [newMasterId, hopNumber, hoprow.name, hoprow.alphaacid, hoprow.droptime, hoprow.weight];
            ...
            res.json({ destination: "recipe", msg: "new data insertion successful!" });
            console.log("insert successful!________________________________________END");
        } catch (err) {
    ...
    .get((req, res) => {
        recipe_db.sqlQuery('SELECT id, name FROM recipe_master', [], 'all')
        .then((rows) => {
          res.status(200).json(rows);
        })
        .catch((err) => {
          console.log(err);
          res.status(500).json({destination:"dropdown", msg: 'Internal server error while getting data'});
        })
    });
